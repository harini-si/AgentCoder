...
class Game:
    def __init__(self):
        ...
        self.all_sprites.add(self.paddle_human)
        self.paddle_cpu = PaddleCPU(self)
        self.all_sprites.add(self.paddle_cpu)
        self.score_human = 0
        self.score_cpu = 0
    ...
    def run(self, event):
        ...
        if pygame.sprite.spritecollide(self.ball, [self.paddle_human], False):
            self.ball.speed[0] = -self.ball.speed[0]

        if pygame.sprite.spritecollide(self.ball, [self.paddle_cpu], False):
            self.ball.speed[0] = -self.ball.speed[0]
        ...
        if self.score_human >= 5 or self.score_cpu >= 5:
            ...
            self.screen.blit(restart_text, (WIDTH // 2 - 140, HEIGHT // 2 + 20))
